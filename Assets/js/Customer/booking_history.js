/**
 * Booking History - Cancel Seats Modal & AJAX
 */
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Open modal
    document.querySelectorAll('.btn-cancel-seats').forEach(function(btn) {
        btn.addEventListener('click', function() {
            const bookingId = this.dataset.bookingId;
            const canCancel = this.dataset.canCancel === '1';
            ...
            // Handle 30-min restriction
            const warningEl = document.getElementById('cancelTimeWarning');
            if (!canCancel) {
    ...
    function updateSubmitState() {
        ...
        const canCancel = document.querySelector('.btn-cancel-seats[data-booking-id="' + document.getElementById('cancelBookingId').value + '"]');
        
        if (canCancel && canCancel.dataset.canCancel === '1') {
    ...
    if (cancelForm) {
        cancelForm.addEventListener('submit', function(e) {
            ...
            const confirmMsg = 'Are you sure you want to cancel seat' + (seatNames.length > 1 ? 's' : '') + ' ' + seatNames.join(', ') + '?';
