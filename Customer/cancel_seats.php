<?php
/**
 * Partial Seat Cancellation Endpoint
 * POST - AJAX JSON endpoint
 * Input: booking_id (int), seat_ids[] (array of show_seat_id values)
 */
session_start();
...
$stmt = $conn->prepare("
    SELECT b.booking_id, b.booking_status, b.show_id,
           s.show_date, s.show_time, s.movie_id
    FROM bookings b
...
// 30-minute backend validation
$show_datetime = $booking['show_date'] . ' ' . $booking['show_time'];
$show_ts = strtotime($show_datetime);
$now_ts = time();
$remaining_minutes = ($show_ts - $now_ts) / 60;

if ($remaining_minutes <= 30) {
    echo json_encode([
        'success' => false,
        'message' => 'Cancellation failed. Bookings cannot be cancelled within 30 minutes of the scheduled show.'
    ]);
    exit;
}
...
try {
    ...
    if (count($seat_names) === 1 && $total === 1) {
        ...
        $remarks = "Full cancellation (Seats: $seats_str)";
    } else {
        $remarks = "Partial cancellation (Seats: $seats_str)";
    }
    ...
    echo json_encode([
        'success' => true,
        'message' => 'Selected seats have been cancelled successfully.',
